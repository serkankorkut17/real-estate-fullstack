...
const EditMyProperty = () => {
			...
			try {
				...
				if (response.success) {
					...
					if (ownerId !== String(user.id)) {
						...
						toast.error(
							t("propertyForm.errors.authError")
						);
					...
					const formattedStartDate = property.startDate
						? format(new Date(property.startDate), "yyyy-MM-dd")
						: "";
					const formattedEndDate = property.endDate
						? format(new Date(property.endDate), "yyyy-MM-dd")
						: "";
					...
					setExistingImages(property.media || []);
					setLocationInfo({
					...
					setError(t("propertyForm.errors.loadingError"));
				}
			} catch (error) {
	...
	const removeExistingImage = async (index, imageData) => {
		try {
			...
			if (response.success) {
				...
				toast.success(t("propertyForm.validation.fileTypeError"));
			} else {
				setError(response.error?.message || t("propertyForm.errors.deleteImageError"));
				toast.error(t("propertyForm.errors.deleteImageError"));
			}
		} catch (error) {
			console.error("Error deleting existing image:", error);
			setError(t("propertyForm.errors.deleteImageError"));
			toast.error(t("propertyForm.errors.deleteImageError"));
		} finally {
			setLoading(false);
		}
	...
	const handleSubmit = async (e) => {
		...
		try {
			...
			const response = await updateProperty(id, form);
			if (response.success) {
				setSuccess(t("propertyForm.editProperty.update"));
				setTimeout(() => {
					toast.success(t("propertyForm.editProperty.update"));
					navigate("/user/my-properties");
				}, 1500);
			}
		} catch (err) {
			...
			<div className="min-h-screen bg-gradient-to-br from-blue-50 to-indigo-100 dark:from-gray-900 dark:to-gray-800 flex items-center justify-center">
				<div className="text-center">
					...
					<p className="text-gray-600 dark:text-gray-400">
						{t("propertyForm.editProperty.loadingProperty")}
					</p>
				</div>
			</div>
		);
			...
			<div className="container mx-auto px-4 py-8">
				...
				<div className="text-center mb-8">
					<h1 className="text-4xl font-bold text-gray-900 dark:text-white mb-4">
						✏️ {t("propertyForm.editProperty.title")}
					</h1>
					<p className="text-xl text-gray-600 dark:text-gray-300 mb-6">
						{t("propertyForm.editProperty.description")}
					</p>
				...
				<div className="max-w-6xl mx-auto">
					<div className="grid grid-cols-1 lg:grid-cols-4 gap-8">
							...
							<div className="bg-white dark:bg-gray-800 rounded-xl shadow-lg p-6 sticky top-4">
								...
								<div className="mt-6 pt-6 border-t border-gray-200 dark:border-gray-600">
									...
									>
										...
										{t("propertyForm.editProperty.backToProperties")}
									</Button>
								</div>
							</div>
						</div>
						...
						<div className="lg:col-span-3">
									...
									<div className="bg-gray-50 dark:bg-gray-700 px-6 py-4 flex justify-between items-center">
										...
										{currentStep === steps.length ? (
											...
											>
												...
												{loading
													? t("propertyForm.editProperty.updating")
													: t("propertyForm.editProperty.update")}
											</Button>
										) : (
											<Button
												type="button"
							...
							{/* Help Section */}
							<div className="mt-6 bg-orange-50 dark:bg-orange-900/20 rounded-xl p-6">
								<div className="flex items-start space-x-4">
									<div className="flex-shrink-0">
										<div className="w-8 h-8 bg-orange-100 dark:bg-orange-800 rounded-full flex items-center justify-center">
											<HiHome className="w-4 h-4 text-orange-600 dark:text-orange-400" />
										</div>
									</div>
									<div>
										<h4 className="text-sm font-semibold text-orange-900 dark:text-orange-100 mb-1">
											{t("propertyForm.tips.editTip")}
										</h4>
										<p className="text-sm text-orange-800 dark:text-orange-200">
											{currentStep === 1 &&
												t("propertyForm.tips.editStep1")}
											{currentStep === 2 &&
												t("propertyForm.tips.editStep2")}
											{currentStep === 3 &&
												t("propertyForm.tips.editStep3")}
											{currentStep === 4 &&
												t("propertyForm.tips.editStep4")}
											{currentStep === 5 &&
												t("propertyForm.tips.editStep5")}
											{currentStep === 6 &&
												t("propertyForm.tips.editStep6")}
											{currentStep === 7 &&
												t("propertyForm.tips.editStep7")}
										</p>
									</div>
								</div>
							</div>
